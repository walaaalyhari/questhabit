...
// Define the properties expected by the WelcomeScreen component
interface WelcomeScreenProps {
  isDarkMode: boolean; // Determines the active styling theme
  onSignIn: () => void; // Callback to proceed past the welcome screen into the main app
}

/**
 * WelcomeScreen component displayed to users before they "sign in" or start.
 * Serves as the landing page / onboarding intro to set the gamified tone.
 */
export default function WelcomeScreen({ isDarkMode, onSignIn }: WelcomeScreenProps) {
  return (
    // Full height viewport container centering the welcome panel
    <div className={`min-h-[100dvh] flex items-center justify-center p-6 ${isDarkMode ? 'dark bg-slate-950 font-sans text-slate-100' : 'bg-slate-100 font-sans text-slate-800'}`}>
      ...
      >
        ...
        {/* Main CTA to enter the application */}
        <button 
          onClick={onSignIn}
